	...
	public function render() { 
		...
		foreach($this->options as $value => $text) {
            ...
            if(!empty($this->inline) && $count > 0)
                echo ' ';
            echo '<label class="', esc_attr($labelClass), '"><input id="', esc_attr($this->_attributes["id"]), '-', esc_attr($count), '"', $this->getAttributes(array("id", "value", "checked", "name", "onclick", "required")), ' value="', $this->filter($value), '"';
            if(in_array($value, $this->_attributes["value"]))
                echo ' checked="checked"';
            echo ' onclick="updateChecksort(this, \'', str_replace(array('"', "'"), array('&quot;', "\'"), $text), '\');"/>', wp_kses_post($text), '</label>';
