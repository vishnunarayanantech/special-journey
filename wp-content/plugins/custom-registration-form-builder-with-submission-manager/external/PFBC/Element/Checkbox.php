	...
	public function render() { 
		...
		foreach($this->options as $value => $text) {
			$value = $this->getOptionValue($value);
                        ...
                        echo '<li><span class="rm-pricefield-wrap"> <input id="', esc_attr($this->_attributes["id"]), '-', esc_attr($count), '"', $this->getAttributes(array("id", "value", "checked")), ' value="', $this->filter($value), '"';
			...
			//echo '/> ', $text, ' </label> ';
			echo '/><label for="',esc_attr($this->_attributes["id"]), '-', esc_attr($count),'"><span>', wp_kses_post($text), '</span></label>';
			...
			++$count;
                ...
                if(isset($this->_attributes["rm_is_other_option"]) && $this->_attributes["rm_is_other_option"] == 1){                    
                   ...
                   if(!$other_val)
                   {
                   echo '<input type="checkbox" value="" id="'.esc_attr($this->_attributes["id"]).'_other" name="'.esc_attr($this->getAttribute("name")).'" style="'.wp_kses_post($this->getAttribute("style")).'"><label for="'.esc_attr($this->_attributes["id"]).'_other">'.__('Other','custom-registration-form-builder-with-submission-manager').'</label></li>'.
                   ...
                   {
                    echo '<input type="checkbox" value="" id="'.esc_attr($this->_attributes["id"]).'_other" name="'.esc_attr($this->getAttribute("name")).'" style="'.wp_kses_post($this->getAttribute("style")).'" checked><label for="'.esc_attr($this->_attributes["id"]).'_other">'.__('Other','custom-registration-form-builder-with-submission-manager').'</label></li>'.
