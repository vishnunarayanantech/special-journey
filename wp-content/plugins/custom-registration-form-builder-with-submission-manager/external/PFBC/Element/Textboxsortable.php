...
class Element_Textboxsortable extends Element
{
    ...
    public function renderSortable($type = "prepend",$suffix='')
    {
        ...
        if ($type === "append")
            echo '<div class="rm_actions" onClick ="rm_delete_appended_field(this,rm_sortable_elements_'.esc_attr($suffix).')"><a href="javascript:void(0)">' . RM_UI_Strings::get("LABEL_DELETE") . '</a></div></li>';
        if ($type === "close")
            echo '</ul>';
        if($type === "add_action")
