...
JS;
	...
	public function render() { 
		...
		foreach($this->options as $value => $text) {
			...
			echo '<li> <input id="', esc_attr($this->_attributes["id"]), '-', esc_attr($count), '"', $this->getAttributes(array("id", "value", "checked")), ' value="', esc_attr($this->filter($value)), '"';
			...
			++$count;
                ...
                if(isset($this->_attributes["rm_is_other_option"]) && $this->_attributes["rm_is_other_option"] == 1){                       //get value of "other" field to be prefilled if provided.
                   ...
                   echo '<li>';
                   if($other_val){
                     echo      '<input id="'.esc_attr($this->_attributes["id"]).'_other" type="radio" value="" name="'.$this->getAttribute("name").'" style="'.$this->getAttribute("style").'" checked><label for="'.esc_attr($this->_attributes["id"]).'_other">'.__('Other','custom-registration-form-builder-with-submission-manager').'</label></li>'.
                        '<li id="'.esc_attr($this->_attributes["id"]).'_other_section">'.
                        '<input style="'.$this->getAttribute("style").'" type="text" id="'.esc_attr($this->_attributes["id"]).'_other_input" name="'.$this->getAttribute("name").'" value="'.esc_attr($other_val).'">';
                   }
                   else
                   {
                     echo  '<input id="'.esc_attr($this->_attributes["id"]).'_other" type="radio" value="" name="'.$this->getAttribute("name").'" style="'.$this->getAttribute("style").'"><label for="'.esc_attr($this->_attributes["id"]).'_other">'.__('Other','custom-registration-form-builder-with-submission-manager').'</label></li>'.
                        '<li id="'.esc_attr($this->_attributes["id"]).'_other_section" style="display:none">'.
                        '<input style="'.$this->getAttribute("style").'" type="text" id="'.esc_attr($this->_attributes["id"]).'_other_input" name="'.$this->getAttribute("name").'" disabled>';
