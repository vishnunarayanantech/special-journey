...
class View_UserForm extends View_SideBySide{
    ...
    public function renderElement(Element $element)
    {
        ...
        {               
             echo '<div class="'.esc_attr($row_class).'">', $element->render(), '', $this->renderDescriptions($element), '</div>';            
        } elseif($element instanceof Element_Map )
        {
            ...
            echo '<div class="'.esc_attr($row_class).'">', $this->renderLabel($element), '<div class="'.esc_attr($input_class).'">', $element->render(), '</div>', $this->renderDescriptions($element), '</div>';
            
            
        }
        elseif($element instanceof Element_Captcha )
        {
            echo '<div class="'.esc_attr($row_class).' rm_captcha_fieldrow">', $this->renderLabel($element), '<div class="'.esc_attr($input_class).'">', $element->render(), '</div>', $this->renderDescriptions($element), '</div>';            
        }
        else
        {
            echo '<div class="'.esc_attr($row_class).'">', $this->renderLabel($element);
            echo '<div class="'.esc_attr($input_class).'">', $element->render();
    ...
    public function renderLabel(Element $element)
     {
        ...
        if (!empty($label))
        {
            ...
            echo '<div class="'.esc_attr($field_class).'" for="', $element->getAttribute("id"), '" style="',$element->getAttribute("labelstyle"),'"><label>';
