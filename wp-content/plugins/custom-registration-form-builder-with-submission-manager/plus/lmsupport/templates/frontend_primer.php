...
<?php if(!$data->is_lm_installed): ?>
    <a href="<?php echo esc_url($data->lm_install_url); ?>"> <?php _e('Install Now','custom-registration-form-builder-with-submission-manager'); ?></a>.
<?php elseif(!$data->is_lm_activated): ?>
    <a href="<?php echo esc_url($data->lm_activate_url); ?>"><?php _e('Activate Now','custom-registration-form-builder-with-submission-manager'); ?></a>.
<?php else: ?>
