                ...
                if ($data->submissions):
                    foreach ($data->submissions as $data_single):
                        ...
                        <tr>
                            ...
                            <td class="rm-submission-unique-id"><?php echo !empty($token) ? esc_html($token) : ''; ?></td>
                            <td><?php echo RM_Utilities::localize_time($data_single->submitted_on, $data->date_format); ?></td>
                            <td><i class="material-icons" onclick="document.getElementById('rmsubmissionfrontform<?php echo esc_html($data_single->submission_id); ?>').submit()">cloud_download</i></td>
                        <form id="rmsubmissionfrontform<?php echo esc_html($data_single->submission_id); ?>" method="post">
                            <input type="hidden" value="<?php echo esc_html($data_single->submission_id); ?>" name="rm_submission_id">
                ...
                <ul class="rmpagination">
                    ...
                    for ($i = $first_visible_page; $i <= $last_visible_page; $i++):
                        if ($i != $data->curr_page_sub):
                            ?>
                            <li onclick="get_tab_and_redirect('rm_reqpage_sub=<?php echo esc_html($i); ?>')"><a><?php echo esc_html($i); ?></a></li>
                        <?php else:
                            ?>
                            <li onclick="get_tab_and_redirect('rm_reqpage_sub=<?php echo esc_html($i); ?>')"><a class="active"><?php echo esc_html($i); ?></a></li>
                        <?php
                        endif;
                    endfor;
                    if ($data->total_pages_sub > $max_pages_without_abb):
