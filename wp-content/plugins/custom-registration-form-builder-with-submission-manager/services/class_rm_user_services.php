...
class RM_User_Services extends RM_Services
{
    ...
    public function google_login_callback($token) {
        if(defined('REGMAGIC_ADDON')) {
            $addon_service = new RM_User_Services_Addon();
            return $addon_service->google_login_callback($token,$this);
        }
    }
    ...
    public function social_login_using_email($user_email = null, $user_fname = null, $type = null, $token = null) {
        ...
        switch ($type) {
            case 'facebook':
                ...
                break;
            case 'google':
                $login_success = $this->google_login_callback($_POST['token']);
                break;
            case 'instagram':
                $response = wp_remote_get('https://graph.instagram.com/v12.0/me?fields=id,username&access_token='.$_POST['token']);
                $response = json_decode(wp_remote_retrieve_body($response));
