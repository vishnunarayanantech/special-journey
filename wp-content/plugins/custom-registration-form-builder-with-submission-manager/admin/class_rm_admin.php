...
class RM_Admin {
   ...
   version="1.1"><metadata
    ...
    public function enqueue_scripts() {
        ...
        wp_register_script('chart_js',RM_BASE_URL . 'admin/js/chart.min.js',array('jquery'));
...
//                add_submenu_page("","What's New!","What's New", "manage_options", "rm_whats_new", array($this, 'show_whatsnew'));
    ...
    public function add_version_header() {
        ?>
        <style>
          .rmagic::before {content:"v<?php echo defined('REGMAGIC_ADDON') ? RM_ADDON_PLUGIN_VERSION . " Premium" : RM_PLUGIN_VERSION . " Standard"; ?>"}
    ...
    public function upload_template(){
       ...
       if($_FILES && current_user_can('manage_options')){
               ...
               $status= move_uploaded_file(sanitize_file_name($_FILES['file']['tmp_name']['0']), $name);
               else
               $status= move_uploaded_file(sanitize_file_name($_FILES['file']['tmp_name']), $name);    
